...
//  伪代码
    ...
    val orderWideWithSplitDstream: DStream[OrderDetailWide] = orderDetailWideDStream.mapPartitions { orderWideItr =>
      ...
      for (orderWide <- orderWideList) {
        ...
        val restOriginAmount: Double = orderWide.original_total_amount - originAmountSum
        if (detailOrginAmount == restOriginAmount) {
          //3.1  最后一笔 用减法 ：实付分摊金额= 实付总金额 - （其他明细已经计算好的【实付分摊金额】的合计）
          orderWide.final_detail_amount = Math.round((orderWide.final_total_amount - splitAmountSum) * 100D)/100D
        } else {
    ...
    orderWideWithSplitDstream.map(orderwide=>JSON.toJSONString(orderwide,new SerializeConfig(true)))
//      .print(1000)
    ...
    orderWideKafkaSentDstream.foreachRDD{rdd=>
      ...
      df.write.mode(SaveMode.Append)
        ...
        .jdbc("jdbc:clickhouse://hadoop112:8123/test0105","order_wide_0105",new Properties())
