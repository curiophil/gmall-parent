...
object TrademarkStatApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //加载流
    val sparkConf: SparkConf = new SparkConf().setAppName("trademark_stat_app").setMaster("local[2]")
    ...
    if (kafkaOffsetMap != null && kafkaOffsetMap.size > 0) {
      //    print(kafkaOffsetMap)
      recordInputStream = MyKafkaUtil.getKafkaStream(topic, ssc, kafkaOffsetMap, groupId)
    } else {
      var map: Map[TopicPartition, Long] = Map(new TopicPartition(topic, 0) -> 0, new TopicPartition(topic, 1) -> 0)
      println(map)
      recordInputStream = MyKafkaUtil.getKafkaStream(topic, ssc, map, groupId)
    }
    ...
    val amountWithTmDstream: DStream[(String, Double)] = jsonObjDstream.map(orderWide =>
